...
impl Game {
                ...
                if let Some(command) = self.get_command(interval - now.elapsed()) {
                    match command {
                        ...
                        Command::Turn(towards) => {
                            // Direction needs to be a 90 degree turn
    ...
    // Place the food at a random point on the board that the snake does not occupy
    ...
    // All the rendering stuff is down here!
    fn render(&mut self) {
        self.draw_borders();
    ...
    // Takes in the snake information and draws it on the board
    // Uses the fancy characters to denote the head space, bends, and rest of the body
    // Also changes color based on the speed of the snake
            ...
            let symbol = if let Some(&next) = next {
                if let Some(&previous) = previous {
                    if previous.x == next.x {
                        ...
                        let u = if body.y == 0 { body.clone() }
                        else { body.transform(Direction::Up, 1) };
                        let l = if body.x == 0 { body.clone() }
                        else { body.transform(Direction::Left, 1) };
    ...
    // Draws the food as an '@'
    // Takes the food fields and uses those to print '@' at the corresponding location on the board
    ...
    // Prints blank space everywhere where there is not the snake, food, or the borders
    ...
    // Prints '#' as the border characters around the background of the playing board
    // 'MoveTo' method moves the cursor to that space so that the character can be printed
